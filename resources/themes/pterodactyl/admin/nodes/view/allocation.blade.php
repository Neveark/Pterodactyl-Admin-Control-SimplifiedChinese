...
@section('content-header')
    <h1>{{ $node->name }}<small>控制此节点上对服务器可用的分配。</small></h1>
    <ol class="breadcrumb">
            ...
            <div class="box-body table-responsive no-padding" style="overflow-x: visible">
                <table class="table table-hover" style="margin-bottom:0;">
                    <tr>
                        ...
                        <th>IP地址 <i class="fa fa-fw fa-minus-square" style="font-weight:normal;color:#d9534f;cursor:pointer;" data-toggle="modal" data-target="#allocationModal"></i></th>
                        <th>IP别名</th>
                        <th>端口</th>
                        <th>已分配至...</th>
                        <th>
    ...
    <div class="col-sm-4">
        <form action="{{ route('admin.nodes.view.allocation', $node->id) }}" method="POST">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">分配新的地址</h3>
                </div>
                <div class="box-body">
                    <div class="form-group">
                        <label for="pAllocationIP" class="control-label">IP地址</label>
                        <div>
                            ...
                            <p class="text-muted small">输入一个IP地址以在此分配端口。</p>
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="pAllocationIP" class="control-label">IP别名</label>
                        <div>
                            <input type="text" id="pAllocationAlias" class="form-control" name="allocation_alias" placeholder="alias" />
                            <p class="text-muted small">如果您想为这些分配分配默认别名，请在此处输入。</p>
                        </div>
                    </div>
                    <div class="form-group">
                ...
                <div class="box-footer">
                    {!! csrf_field() !!}
                    <button type="submit" class="btn btn-success btn-sm pull-right">确定</button>
                </div>
            </div>
        </form>
    </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">为IP块删除分配</h4>
            </div>
    ...
    function deleteSelected() {
        ...
        if (selectedItems.length !== 0) {
            ...
            swal({
                type: 'warning',
                title: '',
                text: '你确定要删除这些分配: ' + formattedItems + '吗？',
                html: true,
                showCancelButton: true,
